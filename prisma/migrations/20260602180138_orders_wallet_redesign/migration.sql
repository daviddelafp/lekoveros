-- 1. Drop unused tables (PoolItem before PurchasePool due to FK)
DROP TABLE IF EXISTS "PoolItem";
DROP TABLE IF EXISTS "PurchasePool";
DROP TYPE IF EXISTS "PoolStatus";

-- 2. Remove WishlistItem.status BEFORE dropping the type that depends on it
...
-- 3. Now safe to drop WishlistStatus (no more dependents)
DROP TYPE IF EXISTS "WishlistStatus";

-- 4. Add wallet balance to User
ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "walletBalance" DECIMAL(10,2) NOT NULL DEFAULT 0;

-- 5. Fix CardPhoto: drop old FK and column, purge orphaned rows
...
-- 9. Add orderItemId to CardPhoto (table is empty — NOT NULL safe)
